...
####################### Two sample MR for eQTL and Alzheimer ################
...
setwd("~/Desktop/MPhil-GM-diss-crd59-2024-main/ADAMTS4")
...
write.csv(H_data, "./IVs_eQTL_Alzheimer.csv")


################################ PRIMARY MR ###################################

# Perform MS and generate odds ratios

mr_results <- mr(H_data)
mr_results <- generate_odds_ratios(mr_results)
write.csv(mr_results, "/mr_results_eQTLs_AD_ADAMTS4.tsv")

# Calculate pleiotropy

pleiotropy <- mr_pleiotropy_test(H_data)
write.csv(pleiotropy, "ADAMTS4_Pleiotropy.csv")

#Q statistics for heterogeneity

heterogeneity <- mr_heterogeneity(H_data, method_list=c("mr_egger_regression", "mr_ivw"))
write.csv(heterogeneity, "heterogeneity_eQTL_ADAMTS4.csv")


#### MR Results plot, figure 19A

# Create the middle part of the figure
...
figure_middle <- 
  ...
  labs(x="", y="") +
  coord_cartesian(ylim=c(1,6), xlim=c(0.9, 1.3)) +
...
left_figure <-
  table_for_plot  |>
  ggplot(aes(y = method)) + #grab MR method results
  geom_text(aes(x = 0, label = c("MR Egger", "Weighted median", "IVW", "Simple mode", "Weighted Mode" , "MR method")), hjust = 0, 
...
plot <- left_figure + figure_middle + right_figure + plot_layout(design = layout) +
  ggtitle("ARSA IVs cis-eQTLs and Alzheimer MR results") +
...
# SCATTER PLOT, figure 19B

scatter <- trything(mr_results, H_data)

ggsave(scatter[[1]], file="scatter_plot_eQTLs_Alzheimer.pdf", device = "pdf", width=9, height=4)


############### SINGLE SNP ANALYSIS ########################################

# Run single SNP MR and generate odds ratios

res_single <- mr_singlesnp(H_data)
res_single <- generate_odds_ratios(res_single)
write.csv(res_single, file="res_single_eQTL_Alzheimer.csv")


#### Create figure 19C

# Start with middle section
...
table_for_plot_single$p[1:3] <- 1
table_for_plot_single$p[5:6] <- 1
...
left_figure_single <-
  ...
  geom_text(aes(x = 0, label = c("rs4636483", "rs58514620", 
                                 "rs61804399", "rs79181095", 
...
plot_single <- left_figure_single + figure_middle_single + right_figure_single + plot_layout(design = layout) +
  ggtitle("ARSA IVs cis-eQTLs and Alzheimer Single SNP MR results ") +
  theme(plot.title = element_text(size = 14,
                                  hjust = 1.35, 
                                  face = "bold",
