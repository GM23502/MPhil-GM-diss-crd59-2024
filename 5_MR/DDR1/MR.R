...
setwd("~/Desktop/MPhil-GM-diss-crd59-2024-main/DDR1")
...
MS_outcome <-
  read_outcome_data("MS_outcome.csv",
                    sep = ",",
                    snp_col = "SNP",
                    beta_col = "B",
                    se_col = "SE",
...
H_data <- harmonise_data(exposure_dat = eQTL_exposure, outcome_dat = MS_outcome)
...
figure_middle <- 
  mr_results |> 
  ggplot(aes(y = fct_rev(method))) + # have the methods
  theme_classic() +
  ...
  labs(x="", y="") +
  coord_cartesian(ylim=c(1,6), xlim=c(0, 1)) +
...
left_figure <-
  table_for_plot  |>
  ggplot(aes(y = method)) + #grab method estimates
...
plot <- left_figure + figure_middle + right_figure + plot_layout(design = layout) +
  ggtitle("DDR1 IVs cis-eQTLs and MS MR results") +
...
# SCATTER PLOT, figure 17B

scatter <- trything(mr_results, H_data)

ggsave(scatter[[1]], file="scatter_plot_eQTLs_MS.pdf", device = "pdf", width=9, height=4)


############### SINGLE SNP ANALYSIS ########################################

# Perform Single SNP MR and generat odds ratios

res_single <- mr_singlesnp(H_data)
res_single <- generate_odds_ratios(res_single)

write.csv(res_single, file="res_single_eQTL_MS.csv")
...
figure_middle_single <- 
  ...
  geom_vline(xintercept = 1, linetype="dashed") + # Add a vertical line at x = 1
  labs(x="", y="") +
  coord_cartesian(ylim=c(1,6), xlim=c(0, 1.1)) +
  annotate("text", x = -.32, y = 5, label = "") +
  theme(axis.line.y = element_blank(), # taking out the axis lines and text 
        axis.ticks.y= element_blank(),
        axis.text.y= element_blank(),
...
left_figure_single <-
  ...
  geom_text(aes(x = 0, label = c("rs114141192", "rs2233969", "rs3130695", 
                                 "IVW", "MR Egger", "SNP")), hjust = 0, 
            fontface = "bold", size = 3.5) +
  geom_text(
    aes(x = 1, label = OR_CIs), # Add OR and CIs
    hjust = 0,
...
plot_single <- left_figure_single + figure_middle_single + right_figure_single + plot_layout(design = layout) +
  ...
  theme(plot.title = element_text(size = 14,
                                  hjust = 1.35, 
                                  face = "bold",
                                  margin = margin(t= 0, l = 0, b = 15, r= 75)))
